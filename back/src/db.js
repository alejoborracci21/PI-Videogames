...
fs.readdirSync(path.join(__dirname, '/models'))
  ...
  .forEach((file) => {
    ...
    // Verifica si las exportaciones son funciones antes de ejecutarlas
    for (const key in models) {
      if (Object.prototype.hasOwnProperty.call(models, key)) {
        const modelDefiner = models[key];
        if (typeof modelDefiner === 'function') {
          modelDefiner(sequelize);
        } else {
          console.error(`El modelo ${key} no es una función válida:`, modelDefiner);
        }
      }
    }
  });
